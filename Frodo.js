  ...
  var j = schedule.scheduleJob(rule, async () => {
    ...
    //while(!successful && !timedout) {
        ...
        if(lasttick == undefined) {
          ...
          if(planet != undefined && galaxy != undefined && alliance != undefined && user != undefined) {
            ...
            if(planet[3] != galaxy[3] || galaxy[3] != alliance[3] || planet[3] != alliance[3]) {
              ...
              newtick.id = planet[3].match(/\d+/g).map(Number)[0];
              
              if(newtick.id <= lasttick.id) {
                console.log(`Stale tick found (pt${newtick.id})`);
                return;
              } else {
                ...
                if(thistick != null) {
                  ...
                  if(thistick.id < config.pa.tick.shuffle) {
      ...
      //##############
                  ...
                  await Tick.updateOne({id: thistick.id}, {
                  ...
                  console.log(`Updated pt${thistick.id} stats in: ${Date.now() - start_time}ms\n`);
