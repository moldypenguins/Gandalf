      ...
      if (!argv.overwrite && dump_tick <= last_tick) {
        ...
        for (let p_temp in planets) {
          ...
          await Planet.updateOne({planet_id: planet.planet_id}, {
            ...
            score_rank: await PlanetDump.find({score:{$gt:planets[p_temp].score},x:{$ne:200}}).countDocuments(),
            value_rank: await PlanetDump.find({value:{$gt:planets[p_temp].value},x:{$ne:200}}).countDocuments(),
            xp_rank:    await PlanetDump.find({xp:{$gt:planets[p_temp].xp},x:{$ne:200}}).countDocuments(),
