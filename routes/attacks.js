...
router.get('/:hash', attackLimiter, async (req, res, next) => {
  ...
  for(let i = 0; i < targs.length; i++) {
    ...
    targs[i].bashlimit = req.session.member.planet && targs[i].value <= req.session.member.planet.value * config.pa.bashlimit.value && targs[i].score <= req.session.member.planet.score * config.pa.bashlimit.score;
  }
