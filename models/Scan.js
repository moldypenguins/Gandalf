        ...
        if(saved !== undefined) {
          ...
          switch(scan_type) {
            case 1:
              ...
              let cr = page_content.match(/<tr><td[^>]*>Crystal<\/td><td[^>]*>([,\d]+)<\/td><td[^>]*>([,\d]+)<\/td><\/tr>/i);
              pscan.roid_crystal = cr[1].replace(/,/g, '');
              pscan.res_crystal = cr[2].replace(/,/g, '');
              let eo = page_content.match(/<tr><td[^>]*>Eonium<\/td><td[^>]*>([,\d]+)<\/td><td[^>]*>([,\d]+)<\/td><\/tr>/i);
              pscan.roid_eonium = eo[1].replace(/,/g, '');
              pscan.res_eonium = eo[2].replace(/,/g, '');
              //let sv = page_content.match(/<tr><th[^>]*>Value<\/th><th[^>]*>Score<\/th><\/tr>\n\t<tr><td[^>]*>([,\d]+)<\/td><td[^>]*>([,\d]+)<\/td><\/tr>/i);
              let ag = page_content.match(/<tr><th[^>]*>Agents<\/th><th[^>]*>Security\s+Guards<\/th><\/tr>\n\t<tr><td[^>]*>([,\d]+)<\/td><td[^>]*>([,\d]+)<\/td><\/tr>/i);
              pscan.agents = ag[1].replace(/,/g, '');
              pscan.guards = ag[2].replace(/,/g, '');
              let fac = page_content.match(/<tr><th[^>]*>Light<\/th><th[^>]*>Medium<\/th><th[^>]*>Heavy<\/th><\/tr>\n\t<tr><td[^>]*>([^<]+)<\/td><td[^>]*>([^<]+)<\/td><td[^>]*>([^<]+)<\/td><\/tr>/i);
              pscan.factory_usage_light = fac[1];
              pscan.factory_usage_medium = fac[2];
              pscan.factory_usage_heavy = fac[3];
              let pr = page_content.match(/Total Amount of Resources in Production: <span[^>]*>([,\d]+)<\/span>/i);
              pscan.prod_res = pr[1].replace(/,/g, '');
              let ssr = page_content.match(/Total Amount of Resources From Ships Being Sold: <span[^>]*>([,\d]+)<\/span>/i);
          ...
          let requests = await ScanRequest.find({active:true, planet_id:saved.planet.planet_id, scantype: saved.scantype});
          if (requests && requests.length > 0) {
            ...
            for(let request of requests) {
              ...
              let planet = await Planet.findOne({planet_id: request.planet_id});
              if (planet !== null) {
