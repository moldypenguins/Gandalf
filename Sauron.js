...
Mordor.connection.once("open", () => {
  ...
  app.use(async (req, res, next) => {
    ...
    if(req.session?.member !== undefined && req.session.member != null) {
      ...
      let w = 'this.landtick >= 747';// + (res.locals.tick.tick - CFG.alliance.attack.after_land_ticks).toString() + res.locals.member.isCMDR ? '' : ' && this.releasetick <= ' + res.locals.tick.tick.toString();
