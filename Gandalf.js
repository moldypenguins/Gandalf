...
Mordor.connection.once("open", () => {
  ...
  bot.use(async(ctx, next) => {
    ...
    if(ctx?.message?.chat !== undefined && ctx.message.chat.type !== 'private') {
      await TelegramGroup.findOneAndUpdate({_id:Mordor.Types.ObjectId(), telegram_group_id: ctx.message.chat.id.toString(), telegram_title: ctx.message.chat.title, telegram_type: ctx.message.chat.type});
    }
    //parse user
    if(ctx?.message?.from !== undefined && !ctx.message.from.is_bot) {
      await TelegramUser.findOneAndUpdate({_id:Mordor.Types.ObjectId(), telegram_id: ctx.message.from.id, telegram_first_name: ctx.message.from.first_name, telegram_last_name: ctx.message.from.last_name, telegram_username: ctx.message.from.username, telegram_language_code: ctx.message.from.language_code});
    }
    next();
  });
