...
(function (w) {
    ...
    function init() {
        ...
        if (page === 'scan' || page === 'waves') {
            let wvs = ($j('a[target="scan"]').map((id, scn) => getScanId(scn.search))).toArray();
            postScanLinks([...new Set(unique(wvs))]);
        } else if (page === 'alliance_scans') {
            let scns = ($j('a[target="wavescan"]').map((id, scn) => getScanId(scn.search))).toArray();
        ...
        $j.each($j('table').children('tbody').children('tr:not(:first)'), function (ind, obj) {
            if ($j(obj).children('td').length > 3) {
                if (parseInt($j(obj).children('td')[3].innerText) == currETA) {
                    ...
                    if(ind !== 0) {$('<tr><td colspan="' + $j(obj).children('td') + '"></td></tr>').insertBefore(obj);}
    ...
    function postScanLinks(scan_ids, callback) {
